...
if marker_streams:
    print("Found a Marker stream!!!")
    inlet_marker = StreamInlet(marker_streams[0])
    marker_time_correction = inlet_marker.time_correction()
else:
    inlet_marker = False
    print("*** Cannot find Markers stream ***")
print("="*40)
...
while (time() - t_init) < options.duration:
    try:
        data, timestamp = inlet.pull_chunk( timeout=1.0, max_samples=12 ) # pull EEG data
        ...
        if inlet_marker:
            marker, timestamp = inlet_marker.pull_sample(timeout=0.0) # pull Marker data
            if timestamp:
